...
app.post('/find-seed', async (c) => {
  const {
    ...
    d,
    s,
  }: { h: number; a: number; b: number; c: number; d: number; s: number } = await c.req.json()
...
const loadTogepiiWasmModule = async (): Promise<TogepiiWasmModule> => {
  ...
  try {
    ...
    const { iter_smoke, find_seed, search_togepii, new_blink } = instance.exports as {
      iter_smoke: (seed: number, take: number) => void
      find_seed: (h: number, a: number, b: number, c: number, d: number, s: number) => void
      search_togepii: (
        seed: number,
...
const executeSearchTogepii = async (
  ...
  return new Promise((resolve) => {
    ...
    wasmModule.delegates.search_togepii = (
      f_blink: number,
      seed_blink: number,
      f_smoke: number,
      seed_smoke: number,
...
const executeFindSeedByBlink = async (
  ...
  return new Promise((resolve) => {
    ...
    wasmModule.find_seed_by_blink(seed, framesRange[0], framesRange[1], blink.cooltime, blink.tolerance, builder)
